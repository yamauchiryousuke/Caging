%% make mccr_oct
for i = -0.25*0.5:0.125-0.5:0.25*0.5
    for j = -0.25*0.5:0.125*0.5:0.25*0.5
        for k = 1:size(check_boundary(:,1))
        polygon_t = translate(polygon, ...
            [check_boundary(k,1)+j,check_boundary(k,2)+i]);
        center_point = [check_boundary(k,1)+j,check_boundary(k,2)+i,0];
