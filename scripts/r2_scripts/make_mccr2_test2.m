...
c_cls = get_C_closure_object2d_oct(file_name,robot_r,robot_corners,dots_spacing,c_cls_search_interval);  %C_cls_obj作成
...
for i = 0:(object_Y/dcc_search_interval)+1
    for j = 0: 2*(object_X/dcc_search_interval)+1
        polygon_t = translate(polygon, ...
            [dcc_search_interval*(j-(1+(object_X/dcc_search_interval))), ...
            dcc_search_interval*(i-(1+(object_Y/dcc_search_interval)))]);
        center_point = [dcc_search_interval*(j-(1+(object_X/dcc_search_interval))), ...
            dcc_search_interval*(i-(1+(object_Y/dcc_search_interval))),0];
