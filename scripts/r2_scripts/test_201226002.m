...
diary(strcat('result/log_',sprintf(datestr(datetime('now'),'yyyy_mm_dd_HH_MM_ss_FFF')),'.txt'));
...
for i = 1 : size(theta,2)    
    ...
    disp(sort(cellstr(properties(polyout_before_regions))))
    
    n_array = [n_array;dc, target_theta];
    ...
    for num = 1 : show_num
        if num == 1
            plot(polyout_after_regions.(matlab.lang.makeValidName(name_list(num,:))),'FaceColor',prism_c(num,:),'FaceAlpha',0.2,'EdgeColor' ,'black');
            disp(numboundaries(polyout_after_regions.(matlab.lang.makeValidName(name_list(num,:)))))
        end
    end
...
figure
polarscatter(deg2rad(n_array(:,2)),n_array(:,1),20,'filled','MarkerFaceColor','black')    